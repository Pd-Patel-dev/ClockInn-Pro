...
export default function MyLogsPage() {
  ...
  const fetchEntries = async () => {
    ...
    try {
      ...
      const response = await api.get(`/time/my?${params.toString()}`)
      setEntries(response.data.entries || [])
      setTotal(response.data.total || 0)
    } catch (error: any) {
      logger.error('Failed to fetch time entries', error as Error, { endpoint: '/time/my' })
  ...
  const calculateHours = (entry: TimeEntry) => {
    // Use rounded_hours from API if available
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${style.bg} ${style.color}`}>
        {capitalizedStatus}
      </span>
    )
  }
  ...
  return (
    <Layout>
      <div className="px-4 py-8 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-slate-900 mb-1">My Time Logs</h1>
          <p className="text-sm text-slate-600">View and track your time entries</p>
        </div>
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-4 mb-6">
          <h2 className="text-sm font-medium text-slate-900 mb-3">Filter by Date</h2>
