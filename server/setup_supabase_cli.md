...
If you prefer not to link, you can push migrations directly using your database URL:

```bash
supabase db push --db-url "postgresql://postgres:[PASSWORD]@db.xxxxx.supabase.co:5432/postgres" --password YOUR_PASSWORD
```

**Note**: The `supabase db push` command automatically applies ALL migrations in `supabase/migrations/` in order. You don't need to run them individually.
